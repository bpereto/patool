    ...
    if kwargs:
        if verbosity > 0:
            log_info(
                "    with %s"
                % ", ".join(
                    "%s=%s" % (k, shell_quote(str(v))) for k, v in kwargs.items()
                )
            )
        if kwargs.get("shell"):
...
def run_checked(cmd, ret_ok=(0,), **kwargs):
    ...
    if ret.returncode not in ret_ok:
        if "password" in str(ret.stderr):
            raise exceptions.PatoolPasswordError(
...
def check_new_filename(filename):
    ...
    if os.path.exists(filename):
        raise exceptions.PatoolOperationError(
            "cannot overwrite existing file `%s'" % filename
        )
...
def check_archive_filelist(filenames):
    ...
    if not filenames:
        raise exceptions.PatoolOperationError(
            "cannot create archive with empty filelist"
        )
...
def strsize(b, grouping=True):
    ...
    if b < 1024:
        return "%sB" % locale.format("%d", b, grouping)
    if b < 1024 * 10:
        return "%sKB" % locale.format("%d", (b // 1024), grouping)
    if b < 1024 * 1024:
        return "%sKB" % locale.format("%.2f", (float(b) / 1024), grouping)
    if b < 1024 * 1024 * 10:
        return "%sMB" % locale.format("%.2f", (float(b) / (1024 * 1024)), grouping)
    if b < 1024 * 1024 * 1024:
        return "%sMB" % locale.format("%.1f", (float(b) / (1024 * 1024)), grouping)
    if b < 1024 * 1024 * 1024 * 10:
        return "%sGB" % locale.format(
            "%.2f", (float(b) / (1024 * 1024 * 1024)), grouping
        )
    return "%sGB" % locale.format("%.1f", (float(b) / (1024 * 1024 * 1024)), grouping)
...
I can work with ;) .
"""
        % dict(app=configuration.AppName, url=configuration.SupportUrl),
        file=out,
    )
    if etype is None:
    ...
    print(
        os.linesep,
        "******** %s internal error, over and out ********" % configuration.AppName,
        file=out,
    )
